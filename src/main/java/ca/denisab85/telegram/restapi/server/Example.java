...
//
package ca.denisab85.telegram.restapi.server;
  ...
  private static void onAuthorizationStateUpdated(TdApi.AuthorizationState authorizationState) {
    ...
    switch (Example.authorizationState.getConstructor()) {
      ...
      case TdApi.AuthorizationStateWaitEmailAddress.CONSTRUCTOR: {
        ...
        client.send(new TdApi.SetAuthenticationEmailAddress(emailAddress),
            new AuthorizationRequestHandler());
        break;
      }
      ...
      case TdApi.AuthorizationStateClosed.CONSTRUCTOR:
        print("Closed");
        if (!needQuit) {
          client = Client.create(new UpdateHandler(), null,
              null); // recreate client after previous has closed
        } else {
          canQuit = true;
        }
        break;
    ...
    try {
      switch (commands[0]) {
        ...
        case "lo":
          ...
          break;
        case "gch":
          client.send(new TdApi.GetChatHistory(getChatId(commands[1]), 0, 0, 100, false),
              defaultHandler);
          break;
        case "q":
          needQuit = true;
  ...
  private static void getMainChatList(final int limit) {
    synchronized (mainChatList) {
      if (!haveFullMainChatList && limit > mainChatList.size()) {
        ...
        client.send(new TdApi.LoadChats(new TdApi.ChatListMain(), limit - mainChatList.size()),
      ...
      System.out.println();
      System.out.println(
          "First " + limit + " chat(s) out of " + mainChatList.size() + " known chat(s):");
